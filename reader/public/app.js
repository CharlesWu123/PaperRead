(function () {
  ...
  // KaTeX / mermaid come from a CDN; without network the raw text stays readable.
  function typeset(el) {
  ...
  function fillMeta(doc) {
    ...
    document.getElementById('meta-sub').textContent = doc.fullTitle ? (doc.subtitle || '') : (doc.subtitle || '');
    var links = [];
    if (doc.arxivId) {
    ...
    if (doc.pdf) {
      links.push('<a class="btn" target="_blank" rel="noopener" href="/file/' +
        doc.pdf.split('/').map(encodeURIComponent).join('/') + '">本地 PDF</a>');
    }
    links.push('<span class="path-chip" title="源文件">' + esc(doc.path) + '</span>');
  ...
  function openHtmlDoc(doc, opts) {
    ...
    document.getElementById('meta-links').innerHTML =
      '<a class="btn" target="_blank" rel="noopener" href="' + esc(url) + '">新窗口打开</a>';
  ...
  function openDoc(doc, opts) {
    ...
    var q = '/api/doc?id=' + encodeURIComponent(doc.id);
    if (doc.plainPath) q += '&variant=' + variant;
    fetch(q)
  ...
  // Follow relative .md links inside a report to the corresponding doc.
  ...
  searchInput.addEventListener('input', function () {
    ...
    searchTimer = setTimeout(function () {
      fetch('/api/search?q=' + encodeURIComponent(q))
        .then(function (r) { return r.json(); })
        .then(function (d) { renderSearch(d.hits || [], q); })
        .catch(function (e) { showError('搜索出错：' + e.message); });
    }, 220);
  });
